...
$sql1=mysqli_query($con,"SELECT * FROM pharmacist WHERE username='$user'")or die(mysqli_error());
 $result=mysqli_fetch_array($sql1);
    ...
    if($result>0){
$message="<p style='color:blue'>sorry the username entered already exists</p>";
 }else{
$sql="INSERT INTO pharmacist(first_name,last_name,staff_id,postal_address,phone,email,username,password,date)
VALUES('$fname','$lname','$sid','$postal','$phone','$email','$user','$pas','$date')";
if(mysqli_query($con,$sql)) {header("location:http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/admin_pharmacist.php");
}else{
$message1="<p style='color:blue'>Registration Failed, Try again</p>";
}
	}}
?>
<!DOCTYPE html>
